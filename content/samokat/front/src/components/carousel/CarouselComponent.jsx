...
const CarouselComponent = ({ items, type }) => {
    ...
    useEffect(() => {
        ...
        carouselWidth - wrapperWidth < 20 ? setVisibleArrow(false) : setVisibleArrow(true)
    }, [offset])
    ...
    return (
        <div ref={storiesWrapperRef} className={`main_block stories-wrapper relative flex`}>
            ...
            <div ref={storiesRef} className={`stories overflow-x-hidden`}>
                {type === 'story' && items.map((el, idx) => <div key={idx} className={`story rounded`} style={{ marginLeft: `${idx === 0 && `-${offset}px`}`, backgroundImage: `url("https://cm.samokat.ru/processed/story_media/23c1b650-ef72-481c-a89a-255d8d6ac775.jpg")` }}>
                    <span className="text-white font-semibold">Привезём горячую еду</span>
                </div>)}
                {type === 'product' && items.map((el, idx) => <div className="transition-all" style={{ marginLeft: `${idx === 0 && `-${offset}px`}` }}><ProductCardComponent key={idx} data={el} isInCart={false} /></div> )}
            </div>
