...
import { setCountCartitems, setCartPrice } from '../../redux/slices/cartSlice';
...
const HeaderComponent = () => {
  ...
  const { cart , countCartitems, cartPrice} = useSelector(state => state.cart)
  useEffect(() => {
    dispatch(setCountCartitems())
      ...
      <div className={`${styles.headerLogo}`}>
        <Link className='flex items-center' to={'/'}>
          ...
          <div className={`${styles.logoSlogan}`}>
            <h3 className={`${styles.headerTitle}`}>REACT PIZZA</h3>
            <span className={`${styles.headerSubtle}`}>самая вкусная пицца во вселенной</span>
          </div>
        </Link>
      </div>
      {actualUrl.pathname !== '/cart' &&
        <>
          <div className={`${styles.headerSearch}`}>
            <input onChange={(e) => dispatch(setSearchQuery(e.target.value))} value={searchQuery} type="search" placeholder='Сырный цыпленок...' />
          </div>

          <div className={`${styles.headerCart}`}>
            <Link to={'/cart'}>
              <div className={`${styles.headerCartButton}`}>
                <div className={`${styles.headerCartCost}`}>
                  {cartPrice} ₽
                </div>
                <div className={`${styles.headerCartCounter}`}>
                  ...
                  <span>{countCartitems!==0?countCartitems:''}</span>
                </div>
              </div>
            </Link>
          </div>
