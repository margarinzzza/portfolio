      ...
      <div className={`${styles.cart}`}>
        <div className={`flex items-center justify-between`}>
          <div className={`flex items-center`}>
            ...
            <h1 className='ml-2'>Корзина</h1>
          </div>
          <div onClick={() => dispatch(clearCart())} className={`flex items-center ${styles.clearCart}`}>
        ...
        <div className={`${styles.cartList}`}>
          {cart.map((item, idx) => {
            return <CartItemComponent key={item.id} data={item} />
          })}
        </div>

        <div className={`${styles.cartCounterAndSum}`}>
          <span>Всего пицц:<span className='font-bold'> {countCartitems} шт.</span></span>
          <span>Сумма заказа: <span className='font-bold fc-orange'> {cartPrice} ₽</span></span>
        </div>
        <div className={`${styles.pay}`}>
          <Link to={'../'}>
            <div className={`${styles.goBackButton}`}>
              ...
              <span className='ml-4'>Вернуться назад</span>
            </div>
          </Link>
          <div className={`${styles.payNowbutton}`}>Оплатить сейчас</div>
        </div>
      </div>
    </div>
  );
