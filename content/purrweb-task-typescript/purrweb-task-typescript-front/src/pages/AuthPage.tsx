...
const AuthPage = (props: any) => {
  ...
  function passwordFieldManagement(passwordName: any, formType: any) {
    if (!passwordName && !formType?.password) {
      return 'security-input success-check'
    }
    else if (!passwordName && formType?.password) {
      return 'security-input error-input'
    }
    else if (passwordName && formType?.password) {
      return 'error-input '
    }
      ...
      {authState === 'auth' ?
        <form className="auth form-content flex-column d-flex m-auto" onSubmit={handleSubmitLogin(onSubmitLogin)}>
          ...
          <div className="auth-form d-flex flex-column">
            ...
            <div className="password-input">
              ...
              <a onClick={() => setShowPasswordLogin(!showPasswordLogin)} className={showPasswordLogin ? 'password-hide' : 'password-show'}></a>
            </div>
          ...
          <button type="submit">
            <div className={isValidLogin ? 'blue_button  my-3' : 'disabled_button my-3'}>
              Продолжить
            </div>
          </button>
        ...
        : authState === 'register' ?
          ...
          <form className="auth form-content flex-column d-flex m-auto" onSubmit={handleSubmitReg(continueRegister)}>
            ...
            <button disabled={!isValidReg}>
              <div className={isValidReg ? 'blue_button my-3' : 'disabled_button my-3'}>
                Продолжить
              </div>
            </button>
            <div className="text-center">
